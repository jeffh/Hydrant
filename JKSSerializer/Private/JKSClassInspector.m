...
@implementation JKSClassInspector
...
- (NSString *)descriptionForObject:(id)object withProperties:(NSArray *)properties
{
    ...
    for (JKSProperty *property in properties) {
...
- (NSArray *)properties
{
    if (!_properties){
        ...
        for (NSUInteger i=0; i<numProperties; i++) {
            ...
            [properties addObject:[[JKSProperty alloc] initWithName:propertyName
...
- (NSArray *)allProperties
{
    ...
    if (parentClass && parentClass != [NSObject class]) {
        for (JKSProperty *property in [[JKSClassInspector inspectorForClass:parentClass] allProperties]) {
