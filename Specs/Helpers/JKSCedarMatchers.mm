#import "JKSCedarMatchers.h"
#import "JKSError.h"

BeAnError be_an_error() {
    return BeAnError().with_domain(JKSErrorDomain);
}
...
bool BeAnError::matches(const JKSError *error) const {
...
bool BeAnError::domain_matches(const JKSError *error) const {
    return !this->expectedDomain || [this->expectedDomain isEqual:error.domain];
}

bool BeAnError::code_matches(const JKSError *error) const {
    return !this->checkErrorCode || this->expectedErrorCode == error.code;
}

bool BeAnError::fatality_matches(const JKSError *error) const {
    return !this->checkFatality || this->isFatal == error.isFatal;
}

bool BeAnError::subset_of_userinfo_matches(const JKSError *error) const {
...
BeAnError & BeAnError::and_has_source_key(NSString *sourceKey) {
    if (!this->userInfoSubset) {
        this->userInfoSubset = [NSMutableDictionary dictionary];
    }
    this->userInfoSubset[JKSSourceKeyPathKey] = sourceKey;
    return *this;
}

BeAnError & BeAnError::and_has_destination_key(NSString *sourceKey) {
    if (!this->userInfoSubset) {
        this->userInfoSubset = [NSMutableDictionary dictionary];
    }
    this->userInfoSubset[JKSSourceKeyPathKey] = sourceKey;
...
BeAnError & BeAnError::and_has_info(NSDictionary *subset) {
    this->userInfoSubset = [subset mutableCopy];
    return *this;
}
