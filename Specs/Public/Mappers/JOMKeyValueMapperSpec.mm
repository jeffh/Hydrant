...
describe(@"JOMKeyValueMapper", ^{
    ...
    sharedExamplesFor(@"a mapper that converts objects using KVC", ^(NSDictionary *scope) {
        ...
        context(@"when a valid source object is given", ^{
            ...
            it(@"should setup child mappers with itself as the root mapper", ^{
                childMapper1.rootMapperReceived should be_same_instance_as(rootMapper);
                childMapper2.rootMapperReceived should be_same_instance_as(rootMapper);
                if (factory) {
                    childMapper1.factoryReceived should be_same_instance_as(factory);
                    childMapper2.factoryReceived should be_same_instance_as(factory);
                } else {
    ...
    describe(@"parsing the source object", ^{
        ...
        context(@"as the root mapper", ^{
            ...
            itShouldBehaveLike(@"a mapper that converts objects using KVC");
        });
        ...
        context(@"as a child mapper", ^{
            ...
            beforeEach(^{
                ...
                [SpecHelper specHelper].sharedExampleContext[@"rootMapper"] = parentMapper;
